...
create table if not exists monthly_work_reports (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint monthly_work_reports_quantity_check check (
    (work_type_id = 'submitted-documents' and document_count > 0 and work_minutes = 0)
    or
    (work_type_id = 'office-work' and work_minutes > 0 and document_count = 0)
  )
);
...
insert into clients (id, name, code)
values
  ('client-a', '青山商事', 'A001'),
  ('client-b', '北浜物流', 'B002'),
  ('client-c', '中央フーズ', 'C003')
...
values
  ('submitted-documents', 3000, 1, '1件あたり'),
  ('office-work', 500, 10, '10分あたり')
on conflict (work_type_id) do nothing;

update unit_prices set cost_amount = 0 where cost_amount is null;

insert into sorting_unit_prices (id, name, amount, cost_amount, quantity, unit_label)
values
  ('manual', '手入力', 60, 40, 1, '1件あたり'),
  ('smart', 'スマート取込', 40, 0, 1, '1件あたり')
on conflict (id) do nothing;

insert into worker_outsource_prices (worker_id, manual_unit_price, smart_unit_price, submitted_documents_unit_price, office_work_unit_price)
select id, 40, 20, 1000, 300
from workers
on conflict (worker_id) do nothing;

insert into payment_statement_settings (
  id,
  title,
  payment_due_text,
  bank_fee_text,
